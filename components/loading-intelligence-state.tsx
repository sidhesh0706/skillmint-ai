...
export function LoadingIntelligenceState({
  ...
  return (
    <div className="loading-state-shell relative overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-soft sm:p-5">
      ...
      <div className="relative z-10">
        ...
        <div className="loading-state-track mt-4 overflow-hidden rounded-full bg-slate-100">
          <div className="loading-state-progress score-fill h-2 w-full rounded-full bg-gradient-to-r from-mint-500 via-emerald to-cyan" />
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          {steps.map((step, index) => (
            <div
              key={step}
              className="loading-state-step section-reveal flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-[#FAFAF8] px-3 py-2 text-sm font-semibold text-slate-700 shadow-line"
